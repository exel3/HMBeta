<template>
  ...
  <section v-else>
    <article class="userList">
      <div class="titleCard"><p>Lista de grupos baneados</p></div>
      ...
      <table>
        <thead>
          <tr>
            <th>Usuarios en grupos baneados</th>
            <th>Desbanear</th>
          </tr>
        </thead>
        <tbody>
...
<script>
import BaseRow from '@/components/bans/bangroups/BaseRow.vue'
import BanModal from '~/components/bans/bangroups/BanModal.vue'
...
export default {
  ...
  async fetch() {
    ...
    await this.$axios
      .$get(`/api/getAllGroupBans/${this.user.id}`)
      .then((res) => {
        this.usersCurrent = res.groupBans
        this.tableFilter = res.groupBans
      })
      .catch((e) =>
        this.$toasted.show(`Error recuperando los grupos baneados: ${e}`, {
          theme: 'toasted-primary',
          position: 'top-right',
          duration: 10000,
        })
  ...
  methods: {
    ...
    async unBanUser() {
      ...
      await this.$axios
        .$delete(`/api/unBanGroupTable/${idUser}`)
        .then((res) => {
